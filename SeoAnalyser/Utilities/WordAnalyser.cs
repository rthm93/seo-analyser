...
namespace SeoAnalyser.Utilities
{
    public class WordAnalyser : IWordAnalyser, ISeoAnalyser
    {
        ...
        public Task<Dictionary<string, int>> CalculateOccurrences(List<string> splittedInput, List<string> wordsToCount)
        {
            ...
            wordsToCount = PreprocessStrings(wordsToCount);

            var joined = (from w in wordsToCount
                          join s in splittedInput on w equals s into cal
                          from empty in cal.DefaultIfEmpty()
                          group empty by w into grouped
                          select new KeyValuePair<string, int>(grouped.Key, grouped.Count(t => !string.IsNullOrEmpty(t))))
        ...
        public Task<List<string>> SplitWords(string input, bool isRemoveStopWords)
        {
            ...
            {
                results = results.Where(s => results.Except(StopWords).Any(r => r.Equals(s))).ToList();
            }
